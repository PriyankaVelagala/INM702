...
def getLegalNodes(currentX, currentY, maxX, maxY):
    nodes = []
    #check right 
    if currentX + 1 < maxX: nodes.append([currentX+1, currentY])
    #check left
    if currentX - 1 > 0: nodes.append([currentX-1, currentY])
    #check down 
    if currentY + 1 < maxY: nodes.append([currentX, currentY + 1])
    #check up
    if currentY - 1 > 0: nodes.append([currentX, currentY -1])
...
def getLegalNodes(currentX, currentY, maxX, maxY):
    nodes = []
    #check bottom?
    if currentX + 1 < maxY: nodes.append([currentY, currentX+1])
    #check  up?
    if currentX - 1 >= 0: nodes.append([currentY, currentX-1])
    #check right
    if currentY + 1 < maxX: nodes.append([currentY+1, currentX])
    #check left 
